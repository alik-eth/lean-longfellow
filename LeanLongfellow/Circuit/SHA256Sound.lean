...
private theorem nat_mul_sum (s : Finset ℕ) (c : ℕ) (f : ℕ → ℕ) :
  ...
  rw [← smul_eq_mul, ← sum_nsmul]; apply sum_congr rfl; intro x _; rw [smul_eq_mul]

private theorem binary_sum_split (n : ℕ) (b : ℕ → ℕ) :
    ∑ j ∈ Finset.range (n+1), b j * 2 ^ j =
    b 0 + 2 * ∑ j ∈ Finset.range n, b (j+1) * 2 ^ j := by
  rw [Finset.sum_range_succ']; simp only [pow_zero, mul_one]
  have h : ∀ k ∈ Finset.range n, b (k+1)*2^(k+1) = 2*(b (k+1)*2^k) := by intro k _; ring
...
private theorem binary_sum_div (n : ℕ) (b : ℕ → ℕ) (hb : ∀ i, b i ≤ 1) :
  ...
  rw [binary_sum_split, Nat.add_mul_div_left _ _ (by omega : (0:ℕ) < 2),
      Nat.div_eq_of_lt (by have := hb 0; omega)]; simp
...
theorem word32Nat_lt (c : Fin 32 → F) (hc : isWord32 c) : word32Nat c hc < 2 ^ 32 := by
  unfold word32Nat
  have hbound : ∑ i ∈ Finset.range 32, (2:ℕ) ^ i < 2 ^ 32 := by native_decide
  calc ∑ i : Fin 32, (if c i = 1 then 2 ^ (i : ℕ) else 0)
      ≤ ∑ i : Fin 32, 2 ^ (i : ℕ) := Finset.sum_le_sum fun i _ => by
        split
        · exact Nat.le_refl _
        · exact Nat.zero_le _
    _ = ∑ i ∈ Finset.range 32, 2 ^ i := (Fin.sum_univ_eq_sum_range (fun i => 2 ^ i)) 32
    _ < 2 ^ 32 := hbound
...
theorem word32Val_eq_natCast (c : Fin 32 → F) (hc : isWord32 c) :
  ...
  unfold word32Val word32Nat; rw [Nat.cast_sum]
  exact Finset.sum_congr rfl fun i _ => bool_mul_pow_eq_cast (c i) (hc i) i
...
theorem word32_bits_eq_of_val_eq (c₁ c₂ : Fin 32 → F)
  ...
  rw [word32Val_eq_natCast c₁ hc₁, word32Val_eq_natCast c₂ hc₂] at hval
  have hnat : word32Nat c₁ hc₁ = word32Nat c₂ hc₂ :=
...
private theorem nat_cast_combine (a b : ℕ) : (a : F) + (b : F) = ((a + b : ℕ) : F) := by
  push_cast; ring

private theorem nat_cast_mul_pow32 (n : ℕ) :
    (n : F) * (2 : F) ^ 32 = ((n * 2 ^ 32 : ℕ) : F) := by norm_cast
...
theorem word32Add_carry_unique (a b c₁ c₂ : Fin 32 → F) (carry₁ carry₂ : F)
  ...
  have h2ne : (2 : F) ^ 32 ≠ 0 := by
    intro h0
    have key : ((0 : ℕ) : F) = ((4294967296 : ℕ) : F) := by
      push_cast; norm_num at h0 ⊢; exact h0.symm
    exact absurd (hchar 0 4294967296 (by norm_num) (by norm_num) key) (by norm_num)
  have heq : word32Val c₂ + carry₁ * (2:F)^32 = word32Val c₂ + carry₂ * (2:F)^32 := by
    have := h₁.2.2.symm.trans h₂.2.2; rw [hv] at this; exact this
  exact mul_right_cancel₀ h2ne (add_left_cancel heq)

-- Bounded overflow uniqueness (T1, schedule)
...
theorem word32_add_bounded_overflow_eq (c₁ c₂ : Fin 32 → F)
  ...
  have h_nat := hchar _ _
    (by have := word32Nat_lt c₁ hc₁; omega) (by have := word32Nat_lt c₂ hc₂; omega) heq
  exact word32_bits_eq_of_val_eq c₁ c₂ hc₁ hc₂ hchar (by
    rw [word32Val_eq_natCast c₁ hc₁, word32Val_eq_natCast c₂ hc₂]
...
theorem t1_bits_unique (st : SHA256State F) (K_i W_i t1a t1b : Fin 32 → F)
  ...
  exact word32_add_bounded_overflow_eq t1a t1b ova ovb hova hovb ht1a ht1b hchar
    (hova_eq.symm.trans hovb_eq)
...
theorem sha256Round_bits_deterministic (st st₁ st₂ : SHA256State F)
  ...
  cases st₁; cases st₂
  simp only [SHA256State.mk.injEq] at ha he hb1 hb2 hc1 hc2 hd1 hd2 hf1 hf2 hg1 hg2 hh1 hh2 ⊢
  ...
  | _ i ih =>
    ...
    · push Not at hi16
      ...
      exact word32_add_bounded_overflow_eq _ _ c₁ c₂ (by omega) (by omega)
        ...
        (by rw [ih2, ih7, ih15, ih16] at hc₁e; exact hc₁e.symm.trans hc₂e)
...
theorem sha256Compression_val_deterministic [SHA256Constants F]
  ...
  have key : ∀ (k : Fin 65), states₁ k = states₂ k ∧ (states₁ k).isValid := by
    ...
    | succ n ih =>
      ...
      have hround₂ := hcomp₂.2 ⟨n, by omega⟩
      -- The rounds use Fin.castSucc and Fin.succ, which are definitionally equal
      -- to ⟨n, _⟩ and ⟨n+1, _⟩ respectively
      change sha256RoundConstraint (states₁ ⟨n, by omega⟩) (states₁ ⟨n+1, by omega⟩)
        (SHA256Constants.K ⟨n, by omega⟩) (W₁ ⟨n, by omega⟩) _ _ _ _ _ at hround₁
      change sha256RoundConstraint (states₂ ⟨n, by omega⟩) (states₂ ⟨n+1, by omega⟩)
        (SHA256Constants.K ⟨n, by omega⟩) (W₂ ⟨n, by omega⟩) _ _ _ _ _ at hround₂
  ...
  have heq := (key ⟨64, by omega⟩).1; rw [heq]
...
theorem sha256FinalAdd_val_deterministic [SHA256Constants F]
  ...
  have helper : ∀ (a b₁ c₁ : Fin 32 → F) (cr₁ : F) (b₂ c₂ : Fin 32 → F) (cr₂ : F),
      word32Add a b₁ c₁ cr₁ → word32Add a b₂ c₂ cr₂ →
      word32Val b₁ = word32Val b₂ →
      word32Val c₁ = word32Val c₂ := by
    intro a b₁ c₁ cr₁ b₂ c₂ cr₂ h1 h2 hb_eq
    have h1eq := h1.2.2; rw [hb_eq] at h1eq
    exact word32_add_carry_eq c₁ c₂ cr₁ cr₂ h1.1 h2.1 h1.2.1 h2.2.1 hchar
      (h1eq.symm.trans h2.2.2)
  intro i; fin_cases i <;>
    first
    | exact helper _ _ _ _ _ _ _ hf1_0 hf2_0 ha
    | exact helper _ _ _ _ _ _ _ hf1_1 hf2_1 hb
    | exact helper _ _ _ _ _ _ _ hf1_2 hf2_2 hc
    | exact helper _ _ _ _ _ _ _ hf1_3 hf2_3 hd
    | exact helper _ _ _ _ _ _ _ hf1_4 hf2_4 he
    | exact helper _ _ _ _ _ _ _ hf1_5 hf2_5 hf
    | exact helper _ _ _ _ _ _ _ hf1_6 hf2_6 hg
    | exact helper _ _ _ _ _ _ _ hf1_7 hf2_7 hh
